...
$q .= "WHERE (999_id IN (".implode(',', $ids999).") OR `images` LIKE '%null%') AND updated=0 AND active=1";

///$stmt = $db->prepare("SELECT * FROM gh3sp_car_ctlg WHERE id=:id AND updated=0");
$stmt = $db->prepare("SELECT * FROM gh3sp_car_ctlg WHERE (id IN (".$q.") OR id IN (11435,11396,11459,11404,11390,11421,11432)) AND updated=0 LIMIT 2");
    ...
    foreach ($data as $car) {
        ...
        if (is_array($images[0])) {
            ...
            if (!empty($images999)) {
                ...
                $q = "SELECT * FROM gh3sp_adverts WHERE car_id=:id AND updated=0 AND active=1 ORDER BY id";
                $stmt = $db->prepare($q);
