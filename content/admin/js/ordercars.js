...
function getReqPage() {
	// Check URL to determine if we're on ordercars page
	if (window.location.href.indexOf('/ordercars/') !== -1) {
		return 'ordercars';
	}
	// Fallback to ordercars for this file
	return 'ordercars';
}
...
$(document).ready(function(){
	// Initialize sortable on static detail page (order_car.php)
	initPhotoSortable();
	...
	$(document).on('click', '#content_box > .bx .confirm', function(){
		...
		if ( $(this).data('fn')==='edit' ){
			...
			$('#content_box > .id_'+bx_id+' .no_need').each(function(){
				// Checkboxes: read CHECKED state (1/0), not .val() — the value attribute
				// stays "0" until onchange fires, so ticking "out of stock" (n_a) was
				// sent as 0. this.checked is always accurate.
				if (this.type === 'checkbox') { data[$(this).attr('name')] = this.checked ? 1 : 0; }
	...
	$(document).on('click', '#more_it', function(){
		...
		var data = {}; data['tp'] = 'adm'; data['pg'] = 'ordercars'; data['fn'] = 'more';
	...
	$(document).on('input change', '.form-control.required, .form-check-input[required], .form-check-input.contact', function () {
		...
		const radio = wrapper.find('.text-option-radio.order-personal-radio');
...
function collectFormDataSauto($form, bx_id) {
	...
	// Checkboxes are collected from their checked state, not serializeArray():
	// serializeArray() drops an unchecked box entirely and sends the value attribute
	// of a checked one — but that value only updates via onchange, which doesn't
	// always fire (ticking "out of stock" n_a left value="0"). So send both 1 and 0
	// explicitly, and skip these names in the serializeArray pass below.
	const checkboxNames = new Set();
	$form.find('input[type="checkbox"][name]').each(function () {
		if (this.name.endsWith('[]')) return; // skip multi-value (del_img[])
		checkboxNames.add(this.name);
		data.set(this.name, this.checked ? 1 : 0);
	});

	$form.serializeArray().forEach(({ name, value }) => {
		if (checkboxNames.has(name)) return; // handled above
			...
			if (!window._presetsAutoGenerated && schedules.length === 0) {
				...
				setTimeout(function () {
					if ($('#generate_presets').length && schedules.length === 0) {
						// Parsing cars publish 999 ONCE (1×/month × 1 month = a single
						// slot), like auto-publish. Manually-added cars keep the default
						// 1×/month × 3 months. Detect parsing by the hidden parsing_id.
						var isParsing = ($('input[name="parsing_id"]').val() || '') !== ''
							&& $('input[name="parsing_id"]').val() !== '0';
						if (isParsing) {
							$('#preset_frequency').val('1').trigger('change');
							$('#preset_duration').val('1').trigger('change');
						}
