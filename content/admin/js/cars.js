...
$(document).ready(function() {
	...
	if (pending && window.location.href.indexOf('/ctlg') !== -1) {
		try {
			...
			if (data.timestamp && (Date.now() - data.timestamp) < 300000 && data.pg === 'cars') {
				localStorage.removeItem('pending_ai_generation');
				console.log('Triggering AI generation for car ID:', data.car_id);
				var iframe = document.createElement('iframe');
				iframe.style.display = 'none';
				iframe.src = '/ajax.php?tp=adm&pg=cars&fn=ai_generate&car_id=' + data.car_id + '&save_to_db=1&_t=' + Date.now();
...
$(document).ready(function(){
	...
	$(document).on('submit', '#sautoForm', async function (event) {
		...
		try {
			...
			console.error('Actual server response:', error.responseText);
	...
	$(document).on('input change', '.form-control.required, .form-check-input[required], .form-check-input.contact', function () {
		...
		if ($(this).hasClass('sauto-personal-radio')) {
			...
			const selectedText = textarea.length ? textarea.val() : stockPersonalTexts['auto_company'][index].text;
		...
		$.ajax({
			...
			success: function(response) {
				if (response.rtrn?.success) {
					...
					btn.hide();
					stockPersonalTexts['auto_company'][textOption].text = text;
				} else {
...
$(document).ready(function() {
	...
	setTimeout(function() {
		...
		if (dbText) {
			...
			if (savedOption !== '' && savedOption !== undefined) {
				$('.text-option-radio.sauto-personal-radio').prop('checked', false);
				$('.text-option-radio.sauto-personal-radio[value="' + savedOption + '"]').prop('checked', true);
			}
	...
	$(document).on('click', '.delete-all-schedules-btn', function(e) {
		...
		function next(i) {
			...
			$.ajax({
				url: '/ajax.php',
				method: 'POST',
				data: { tp: 'adm', pg: currentPage, fn: 'delete_schedule', schedule_id: sid },
				dataType: 'json',
				success: function(r) {
					if (r && r.success) {
						done++;
