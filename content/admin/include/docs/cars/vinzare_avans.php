...
<div id="p_cont" class="base">
	<div class="pg d1 p1 bg">
		...
		<div class="gr">
			...
			<table class="n1">
				<tr><td class="n1">Marca</td><td class="n2">'.(isset($_POST['br']) ? $_POST['br'] : '').'</td><td class="n1">Model</td><td class="n2">'.(isset($_POST['mo']) ? $_POST['mo'] : '').'</td></tr>
		...
		<div class="gr">
			...
			/*if ( isset($_POST['pay_date']) ){
				foreach ($_POST['pay_date'] as $k => $v){
					if ( $v!='' && (isset($_POST['pay_val'][$k]) && $_POST['pay_val'][$k]>=1) ){
						$rtrn .= '<p><b>3.3.</b> Transferuri de bani, MDL:</p><p><b>3.3.'.($k+1).'.</b> '.date( 'd.m.Y', strtotime( $v ) ).' : '.parseCurr( $_POST['pay_val'][$k] ).'.00 lei</p>';
						break;
					}
				}
			}*/
			
			if ( isset($_POST['pay_val']) ){
				foreach ($_POST['pay_val'] as $k => $v){
					if ( $v!=''){ $rtrn .= '<p><b>3.3.</b> Transferuri de bani, MDL:</p><p><b>3.3.'.($k+1).'.</b> '.$v.( isset($_POST['pay_date'][$k])?' - '.$_POST['pay_date'][$k]:'' ).'</p>'; break; }
				}
			}
	...
	<div class="pg d1 p2 bg">';
		...
		/*if ( isset($_POST['pay_date']) && count($_POST['pay_date'])>1 ){
			$rtrn .= '<div class="gr">';
			foreach ($_POST['pay_date'] as $k => $v){
				if ($k==0){continue;}
				if ( $v!='' && (isset($_POST['pay_val'][$k]) && $_POST['pay_val'][$k]>=1) ){
					$rtrn .= '<p><b>3.3.'.($k+1).'.</b> '.date( 'd.m.Y', strtotime( $v ) ).' : '.parseCurr( $_POST['pay_val'][$k] ).'.00 lei</p>';
				}
			}
			$rtrn .= '</div>';
		}*/
		
		if ( isset($_POST['pay_val']) && count($_POST['pay_val'])>1 ){
			$rtrn .= '<div class="gr">';
			foreach ($_POST['pay_val'] as $k => $v){
				if ($k==0){continue;}
				if ( $v!=''){ $rtrn .= '<p><b>3.3.'.($k+1).'.</b> '.$v.( isset($_POST['pay_date'][$k])?' - '.$_POST['pay_date'][$k]:'' ).'</p>'; }
			}
			$rtrn .= '</div>';
		}
		...
		<div class="gr">
			...
			';
			$p5_2 = '<p><b>5.2.</b> Părţile se obligă să soluționeze orice litigii pe calea negocierilor amiabile. Părțile stabilesc și sunt de acord ca termenul de înaintare a pretențiilor care decurg din prezentul contract, este de 15 zile calendaristice, de la data primirii acestora. în cazul în care Părțile nu ajung la un consens, litigiile sunt soluționate de către o instanță competentă a Republicii Moldova.</p>';
			$p5_3 = '<p><b>5.3.</b> Cumpărătorul exprimă consimțământul pentru prelucrarea și folosirea de către Vânzător și/sau persoana autorizată pentru deservirea tehnică, datelor cu caracter personal (în special, doar ne limitându-se - numele si prenumele, adresa poștală, numărul de telefon , email, locul de munca si etc.) în scopul informării Cumpărătorului și suportului relațiilor de contact în cadrul contractului încheiat.</p>';
			
		$rtrn .= '
			'.( isset($_POST['pay_val']) && count($_POST['pay_val'])>5?'':$p5_2 ).'
			'.( isset($_POST['pay_val']) && count($_POST['pay_val'])>2?'':$p5_3 ).'
		</div>
		<div class="conf">CONFIDENTIAL</div>
	</div>
