    ...
    <?php foreach ($pdo as $r) :
        ...
        <div class="bx<?= $stts ?><?= (!rbac_has_permission($user_role, 'cars', 'update') && !rbac_has_permission($user_role, 'cars', 'delete')) ? ' no-admin-hover' : '' ?>" data-id="<?= $r['id'] ?>">
            <div class="log_sauto_999">
                ...
                <?php endif; ?>
                <?php 
                // Check for all 999.md schedules (pending, published, failed)
                $pending999Schedules = [];
                $failed999Schedule = null;
                $all999Schedules = [];
                try {
                    // Get ALL pending schedules for this car
                    ...
                    // Get the most recent failed schedule for display logic
                ...
                if (!empty($r['999_id']) || !empty($pending999Schedules) || $failed999Schedule || !empty($all999Schedules)) : ?>
                    ...
                    <?php elseif ($failed999Schedule) : ?>
                        <!-- Failed schedule - red icon -->
                        <img src="/media/images/site/logo_999.svg" class="log_999" 
                             style="filter: hue-rotate(0deg) saturate(2) brightness(0.8) contrast(1.2); color: #dc3545;"
                             data-tooltip="<?= $tooltip ?? '' ?>"
                             alt="Failed 999"/>
                    <?php else : ?>
            ...
            <div class="adm_menu">
                <?php if( $r['act'] == 1 ) : ?>
                    ...
                    <a class="btn" href="<?= '/'.$_COOKIE['lang'].'/'.$admin_dir.'/changelog/ctlg?car_id='.$r['id'] ?>" title="Log" style="text-decoration:none;text-align:center;display:flex;align-items:center;justify-content:center;">LOG</a>
                    <?php endif; ?>
