        ...
        <div class="bx<?= $stts ?><?= (!rbac_has_permission($user_role, 'cars', 'update') && !rbac_has_permission($user_role, 'cars', 'delete')) ? ' no-admin-hover' : '' ?>" data-id="<?= $r['id'] ?>">
            ...
            <div class="adm_menu">
                <?php if( $r['act'] == 1 ) : ?>
                    ...
                    <a class="btn" href="<?= '/'.$_COOKIE['lang'].'/'.$admin_dir.'/changelog/ctlg?car_id='.$r['id'] ?>" title="Log" style="font-size:10px;line-height:28px;text-decoration:none;text-align:center;">LOG</a>
                    <?php endif; ?>
