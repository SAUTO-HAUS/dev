...
$query_args = [];
$sql = 'SELECT * FROM '.$prefx.'_car_ctlg WHERE 1=1 AND `act`="1" ';
...
<section class="ctlg">';
    ...
    //$pdo = $db->prepare('SELECT * FROM '.$prefx.'_car_ctlg WHERE `act`=1');
    //$pdo->execute();
    ...
    if ( $user_type=='x2' && $r['loc']!=2 ){continue;}
    ...
    <div class="bx'.$stts.'" data-id="'.$r['id'].'">
        ...
        <div class="prc_wrap">
            <div class="prc" title="'.$lng['w']['prc'].'" '.($r['prc']>100 ? '>'.$r['prc'].' <span>'.$lng['l']['cur'][$r['cur']].'</span>' : 'style="font-size:.8rem;">'.$lng['w']['negociabil'] ).'</div>
    </div>
    </div>';
