...
if ( __post('fn')=='search'||__post('fn')=='more'||__post('fn')=='filter' ){
	...
	require_once( $ajax_folder.'/order_filter.php' );
	require_once( $ajax_folder.'/order_catalog.php' );
	$returnIt = [
        'fn'=>__post('fn'),
        'it_pos'=>$c_id,
        'rtrn'=>$rtrn,
...
elseif ( __post('fn')=='add_new' ){
	require_once($ajax_folder.'/order_add_new.php');
	$returnIt = [
        'fn'=>__post('fn'),
        'sub'=>__post('sub'),
        'rtrn'=>$rtrn
...
elseif ( __post('fn')=='999_catalog' ){
    require_once($ajax_folder.'/order_999_catalog.php');
    $returnIt = [
        'fn'=>__post('fn'),
        'sub'=>__post('sub'),
        'rtrn'=>$rtrn
...
elseif ( __post('fn')=='edit' ){
	require_once($ajax_folder.'/order_edit.php');
...
// ─────────── 2. Generate Facebook message using PublicationService ───────────
    ...
    try {
        ...
        // Log scheduling
        $publicationService->logPublication($it_id, 'on_order', 'facebook', true, "Scheduled for {$scheduled_date} {$schedule_time}");
        ...
        $publicationService->logPublication($it_id, 'on_order', 'facebook', false, 'Scheduling error: ' . $e->getMessage());
    }
