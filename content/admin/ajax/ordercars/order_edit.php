...
if ( $_POST['sub']=='start' ){//*************************************************************************************************************** MAKE OVERLAY
	...
	<div class="bx id_'.$_POST['id'].'" data-bx_id="'.$_POST['id'].'">
		...
		<div class="img_bx">
			...
			<label class="dd_plc">
				<input data-gr="edit" class="f" type="file" multiple="multiple" name="img[]" accept=".jpg,.jpeg,image/jpeg" tabindex="1" />
			...
			<div class="prv imgs ready">
				<div class="its bx">'; //--------------------------------------THIS ITEM PHOTOS
					...
					foreach ($pdo as $p){
						$rtrn .= '
						<div class="it '.$p['id'].' f_img ext" data-id="'.$p['id'].'" this_img="/'.$photo_folder.'/'.$c_path.'/'.$p['it_id'].'/high/'.$p['name'].'.jpg" data-n="'.$i.'" data-pos="'.$p['pos'].'">
