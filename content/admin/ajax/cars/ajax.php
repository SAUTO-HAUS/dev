...
elseif ( __post('fn')=='locate_by_link' ){
	...
	// front-end which catalog it lives in. Mirrors ordercars' locate_by_link.
	...
	if ($ctlgId <= 0) {
		...
		// in_stock cars live on /cars/ctlg, on_order on /ordercars/ctlg — report the
		// right one so the front-end sends the user to the correct catalog.
...
elseif ( __post('fn')=='hide' ) {
    ...
    // --- CHANGELOG: log hide ---
    car_changelog_log($db, $prefx, ['car_id' => __post('id'), 'action' => 'hide', 'catalog_type' => 'cars']);
...
elseif ( __post('fn')=='reveal' ) {
    ...
    car_changelog_log($db, $prefx, ['car_id' => __post('id'), 'action' => 'reveal', 'catalog_type' => 'cars']);
...
elseif (__post('fn')=='delete') {
    ...
    car_changelog_log($db, $prefx, ['car_id' => __post('id'), 'action' => 'delete', 'catalog_type' => 'cars']);
...
elseif ( __post('fn')=='restore' ) {
    ...
    car_changelog_log($db, $prefx, ['car_id' => __post('id'), 'action' => 'restore', 'catalog_type' => 'cars']);
...
elseif ( __post('fn')=='sendToTelegramCars' ){
    ...
    try {
        ...
        $pdo->execute([ 'id' => $it_id, 'telegram_published' => 2 ]); // 2 = scheduled
        
        $returnIt['status'] = true;
        $returnIt['message'] = "Programat pentru publicare Telegram la {$scheduled_date} {$schedule_time}";
        
        // Log scheduling
        $publicationService->logPublication($it_id, 'in_stock', 'telegram', true, "Scheduled for {$scheduled_date} {$schedule_time}");
