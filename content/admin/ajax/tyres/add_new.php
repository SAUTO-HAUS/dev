...
if ( $_POST['sub']=='start' ){//*************************************************************************************************************** MAKE OVERLAY
	...
	$pdo = $db->prepare('SELECT `mo`,`mo_nm` FROM '.$prefx.'_tyre_list WHERE `br`=:br ORDER BY `mo` ASC'); $pdo->execute(array('br'=>$r['br']));
	...
	<div class="bx id_'.$_POST['bx_id'].'" data-bx_id="'.$_POST['bx_id'].'">
		<div class="top">
			...
			<div class="title">'.mb_strtoupper($lng['adm']['new_tyre'], "UTF-8").'</div>
		...
		<div class="main_info">
			...
			$rtrn .= '<div class="checks_cont">';
				$rtrn .= '<label><input type="checkbox" name="c" class="no_need" tabindex="1" onchange="this.value = +this.checked;" value="0"> C</label>';
				$rtrn .= '<label><input type="checkbox" name="n_a" class="no_need" tabindex="1" onchange="this.value = +this.checked;" value="0"> '.$lng['l']['stat']['n_a1'].'</label>';
			...
			//---BRAND---
			$rtrn .= '<select class="brand need" name="br" tabindex="1"> <option value="">'.mb_strtoupper($lng['l']['tyre']['spec']['br'], "UTF-8").'</option>';
			foreach ($it_br as $k => $v){ $rtrn .= '<option value="'.$k.'">'.$v.'</option>'; }
			$rtrn .= '</select>';
		
			//---MODEL---
			$rtrn .= '<select class="model" name="mo" def_text="'.mb_strtoupper($lang_model, "UTF-8").'" tabindex="2"><option value="">'.mb_strtoupper($lng['l']['tyre']['spec']['mo'], "UTF-8").'</option></select>';
			
			//---SEASON---
			$rtrn .= '<select class="season need" name="ss" def_text="'.mb_strtoupper($lng['l']['tyre']['spec']['ss'], "UTF-8").'" tabindex="2">
			<option value="" selected>'.mb_strtoupper($lng['l']['tyre']['spec']['ss'], "UTF-8").'</option>';
			foreach ($lng['l']['tyre']['ss'] as $k => $v){ $rtrn .= '<option value="'.$k.'">'.$v.'</option>'; }
			$rtrn .= '</select>';
			
			//---WIDTH---
			$rtrn .= '<input class="width need nmb" name="w" size="16" tabindex="3" placeholder="***/.. R.." type="text" title="'.mb_strtoupper($lng['l']['tyre']['spec']['w'], "UTF-8").'" value="">';
			
			//---HEIGHT---
			$rtrn .= '<input class="height need nmb" name="h" size="16" tabindex="3" placeholder=".../** R.." type="text" title="'.mb_strtoupper($lng['l']['tyre']['spec']['h'], "UTF-8").'" value="">';
			
			//---INSIDE DIAMETER---
			$rtrn .= '<input class="diameter need nmb" name="d" size="16" tabindex="3" placeholder=".../.. R**" type="text" title="'.mb_strtoupper($lng['l']['tyre']['spec']['d'], "UTF-8").'" value="">';
...
elseif ( $_POST['sub']=='end' ){//*************************************************************************************************************** ADD IT INFO TO THE DB
	...
	$br_nm = 'x'; $mo_nm = 'x'; $_POST['mo'] = '';
	$pdo = $db->prepare('SELECT * FROM '.$prefx.'_tyre_list WHERE `br`=:br AND `mo`=:mo'); $pdo->execute([ 'br'=>$_POST['br'], 'mo'=>$_POST['mo'] ]);
	foreach ($pdo as $r){ $br_nm = $r['br_nm']; $mo_nm = $r['mo_nm']; }
	
	$pdo = $db->prepare('INSERT INTO '.$prefx.'_tyre_ctlg (`br`, `mo`, `br_nm`, `mo_nm`, `w`, `h`, `d`, `c`, `ss`, `txt`, `prc`, `cur`, `n_a`, `p_path`, `date`, `author`, `vis`) 
	VALUES (:br, :mo, :br_nm, :mo_nm, :w, :h, :d, :c, :ss, :txt, :prc, :cur, :n_a, :p_path, :date, :author, "0")');//, `vis`, "0"
	$pdo->execute([
		'br'=>$_POST['br'], 'mo'=>$_POST['mo'], 'br_nm'=>$br_nm, 'mo_nm'=>$mo_nm,
		'w'=>$_POST['w'], 'h'=>$_POST['h'], 'd'=>$_POST['d'], 'c'=>$_POST['c'], 'ss'=>$_POST['ss'],
		'txt'=>$_POST['txt'], 'prc'=>$_POST['prc'], 'cur'=>$_POST['cur'], 'n_a'=>$_POST['n_a'],
	...
	foreach($lang_arr as $v){
		...
		$pdo_ar += [ 'lng_'.$i=>$v, 'tp_'.$i=>'item', 'p1_'.$i=>'tyres', 'p2_'.$i=>$last_id, 'qr_'.$i=>'', 'it_id_'.$i=>$last_id, 'ttl_'.$i=>$_POST['title_'.$v], 'h1_'.$i=>$_POST['h1_'.$v], 'dsc_'.$i=>$_POST['meta_desc_'.$v], 'kwd_'.$i=>$_POST['meta_key_'.$v], 'txt_'.$i=>'' ];
		$i++;
	}
	...
	foreach($lang_arr as $v){
		...
		$pdo->execute(array( 'lng'=>$v, 'tp'=>'item', 'p1'=>'tyres', 'p2'=>$last_id, 'qr'=>'', 'it_id'=>$last_id, 'ttl'=>$_POST['title_'.$v], 'h1'=>$_POST['h1_'.$v], 'dsc'=>$_POST['meta_desc_'.$v], 'kwd'=>$_POST['meta_key_'.$v], 'txt'=>'' ));
	}
	*/
}
