...
if ( $_POST['fn']=='edit_sbmt' ){
	...
	$inf=''; $qu=0;
	$inf_ar = ['br', 'mo', 'vin', 'yr', 'clr', 'prc', 'prc_eur', 'prc_av', 'loc', 'term_livr', 'cntr_fr', 'cntr_to', 'adr_to', 't2pay', 'plate', 'extras'];
	$inf_up_ar = ['vin'];
	...
	//UPDATE INFO CTLG
	$pdo = $db->prepare('UPDATE '.$prefx.'_docs_ctlg SET `cd`=:cd, `inf`=:inf, `u`=:u, `date`=:date WHERE `id`=:id');
	$pdo->execute([ 'cd'=>$it_cd, 'inf'=>$inf, 'u'=>$u_id, 'date'=>$doc_date, 'id'=>$it_id ]);
