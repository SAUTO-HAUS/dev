        ...
        if ( $it_id > 0 ){
            ...
            foreach ($pdo as $r){
                ...
                */
                ...
                $rtrn .= '<div class="big_pht" role="img" aria-label="car '.$r['br_nm'].' '.$r['mo_nm'].' id'.$r['id'].' large photo" data-pos="1" data-cnt="'.$img_cnt.'" style="background-image:url('.$z_src.');" data-src="'.$z_src.'">'.car_share_btn($r['id'], 'cars', $lng).car_fav_btn($r['id'], $lng).car_compare_btn($r['id'], $lng).($img_cnt>1?'<div class="bp-nav bp-left" role="button" aria-label="Anterior"></div><div class="bp-nav bp-right" role="button" aria-label="Următor"></div>':'').'</div>';
                $rtrn .= '</div>';
...
$iconTelegramParams = array(
    ...
    'loc' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18" aria-hidden="true" role="img">
                ...
                <div class="seo_links_block" style="margin: 20px 0 0 0; display: flex; gap: 10px; flex-wrap: wrap; justify-content: center;">
                    <a href="/'.$current_lang.'/cars/'.$brand_series_url.'" target="_blank" class="seo-link-btn" style="display: inline-flex; align-items: center; justify-content: center; gap: 8px; padding: 14px 28px; background: linear-gradient(135deg, #e63946, #d62828); color: #fff; text-decoration: none; font-size: 15px; font-weight: 600; border-radius: 0.5rem; box-shadow: 0 4px 15px rgba(230,57,70,0.35); transition: all 0.3s ease; flex: 1; min-width: 250px; text-align: center;">
                        '.$link_text_series.' '.$r['br_nm'].' '.$model_display.$suffix.'
                    </a>
                    <a href="/'.$current_lang.'/cars/'.$brand_url.'" target="_blank" class="seo-link-btn" style="display: inline-flex; align-items: center; justify-content: center; gap: 8px; padding: 14px 28px; background: linear-gradient(135deg, #e63946, #d62828); color: #fff; text-decoration: none; font-size: 15px; font-weight: 600; border-radius: 0.5rem; box-shadow: 0 4px 15px rgba(230,57,70,0.35); transition: all 0.3s ease; flex: 1; min-width: 250px; text-align: center;">
