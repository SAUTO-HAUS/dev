...
// B2B preferential pricing: wraps the breakdown above and applies the dealer fee
// structure server-side when a partner is logged in.
include_once( _SITE_INCL.'/b2b/b2b_pricing.php' );

// B2B actions on the car page (invoice, send to super admin, save).
include_once( _SITE_INCL.'/b2b/b2b_car_actions.php' );
...
function oc_render_regions($active, $lang, $orderBtnText) {
    ...
    foreach ($regions as $rk => $rv) {
        // Do not offer a region the partner cannot access; the catalog SQL and the
        // detail page enforce it anyway.
        if (function_exists('b2b_can_see_region') && !b2b_can_see_region($rk)) { continue; }
...
// Partner without access to this car's region (flagged in body.php): 403 plus an
// explanatory block instead of the car, inside the normal layout.
if (!empty($GLOBALS['b2b_region_blocked'])) {
    http_response_code(403);
    $GLOBALS['is_404_car'] = true; // head.php marks the page noindex
    echo b2b_restricted_html($_COOKIE['lang'] ?? 'ro');
    return;
}
...
$iconTelegramParams = array(
    ...
    'advance_amount' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18" aria-hidden="true" role="img">
                ...
                if (!empty($r['parsing_id']) && in_array($parsingSrc, ['encar', 'openlane', 'ecarstrade', 'auto1'], true)) {
                    ...
                    if ($srcPriceEur > 0) {
                        // b2b_fuel_code() keeps the hybrid sub-type so customs uses the
                        // right base fuel + discount (same as /calc): hbd = full hybrid
                        // (petrol, 25% off), pih = plug-in petrol (50% off), pid =
                        // plug-in diesel (50% off, diesel excise base).
                        $carForBd = [
