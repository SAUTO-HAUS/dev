	...
	foreach ($results as $r) {
		...
		if (!empty($r['offer_timer_end'])) {
			...
			if ($time_remaining_check > 0) {
				...
				$timer_html_for_image = '<div class="offer-timer" style="position: absolute; bottom: 20px; right: 0.6rem; color: #dc3545; font-weight: bold; font-size: 1rem; padding: 5px 10px; border-radius: 4px; z-index: 10;"><div class="timer-display" data-end-time="'.$r['offer_timer_end'].'">'.sprintf('%02d:%02d:%02d:%02d', $days_check, $hours_check, $minutes_check, $seconds_check).'</div></div>';
			} else {
				...
				$timer_html_for_image = '<div class="offer-timer" style="position: absolute; bottom: 20px; right: 0.6rem; color: #dc3545; font-weight: bold; font-size: 1rem; padding: 5px 10px; border-radius: 4px; z-index: 10;"><div class="timer-display" data-end-time="'.$r['offer_timer_end'].'">'.$expired_text.'</div></div>';
			}
		}
		...
		if (!$is_mobile && !empty($r['offer_timer_end'])) {
			...
			if ($time_remaining > 0) {
				...
				$timer_html = '<div class="offer-timer active" style="position: absolute; top: 16.5rem; right: -1.3rem; color: #dc3545; font-weight: bold; font-size: 0.85rem; padding: 5px 10px; border-radius: 4px; z-index: 10;"><div class="timer-display" data-end-time="'.$r['offer_timer_end'].'">'.sprintf('%02d:%02d:%02d:%02d', $days, $hours, $minutes, $seconds).'</div></div>';
			} else {
				// Timer expired - show expired text
				$expired_text = 'Offer expired';
				if (isset($_COOKIE['lang'])) {
				...
				$timer_html = '<div class="offer-timer" style="position: absolute; top: 16.5rem; right: -1.3rem; color: #dc3545; font-weight: bold; font-size: 0.85rem; padding: 5px 10px; border-radius: 4px; z-index: 10;"><div class="timer-display" data-end-time="'.$r['offer_timer_end'].'">'.$expired_text.'</div></div>';
			}
		}
