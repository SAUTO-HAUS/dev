...
if (!function_exists('car_share_btn')) {
	function car_share_btn($id, $page_type, $lng) {
		...
		$ico = '<svg class="csb-ico" width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">'
				.'<circle cx="7" cy="12" r="2" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/>'
				.'<circle cx="17" cy="6" r="2" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/>'
				.'<circle cx="17" cy="18" r="2" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/>'
				.'<path d="M15 7L8.5 11" stroke="currentColor" stroke-width="2"/>'
				.'<path d="M8.5 13.5L15 17" stroke="currentColor" stroke-width="2"/>'
				.'</svg>';
	...
	foreach ($results as $r) {
		...
		if (!empty($r['offer_timer_end']) && empty($r['n_a'])) {
			...
			if ($time_remaining_check > 0) {
				...
				$timer_html_for_image = '<div class="offer-timer" style="position: absolute; bottom: 2px; right: 2px; color: #dc3545; font-weight: bold; font-size: 1rem; padding: 2px 4px; border-radius: 4px; z-index: 10;"><div class="timer-display" data-end-time="'.$r['offer_timer_end'].'">'.sprintf('%02d:%02d:%02d:%02d', $days_check, $hours_check, $minutes_check, $seconds_check).'</div></div>';
			} else {
		...
		if (!$is_mobile && !empty($r['offer_timer_end']) && empty($r['n_a'])) {
			...
			if ($time_remaining > 0) {
				...
				$timer_html = '<div class="offer-timer active" style="position: absolute; bottom: 2px; right: 2px; color: #dc3545; font-weight: bold; font-size: 0.85rem; padding: 2px 4px; border-radius: 6px; z-index: 4;"><div class="timer-display" data-end-time="'.$r['offer_timer_end'].'">'.sprintf('%02d:%02d:%02d:%02d', $days, $hours, $minutes, $seconds).'</div></div>';
			} else {
				// Timer expired → no "expired" overlay; the car is shown as out of stock
				// via its status badge instead.
				$timer_html = '';
