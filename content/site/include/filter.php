...
if (!isset($t_mp[2]) || $t_mp[2]=='' || $t_mp[2]=='cars'){
	...
	foreach ($f_arr['brmo']['mo'] as $v) {
		...
		<div class="ctrl">';
			...
			echo '
			<div class="btns" data-link="/'.$_COOKIE['lang'].'/cars/" data-def="/'.$_COOKIE['lang'].'/cars">
				<div class="btn unst">
					<div class="img"></div>
				...
				<a class="btn sbmt" href="/'.$_COOKIE['lang'].'/cars'.$q_uri.'" data-gr="'.(isset($_GET['gr'])?$_GET['gr']:'').'" data-br="'.(isset($_GET['br'])?str_replace('_','-',$_GET['br']):'').'" data-mo="'.(isset($_GET['mo'])?str_replace('_','-',$_GET['mo']):'').'" data-bt="'.(isset($_GET['bt'])?$_GET['bt']:'').'" data-srt="'.(isset($_GET['srt'])?$_GET['srt']:'').'"';
	...
	// Sort dropdown for cars page (catalog + sort criteria in one select)
