...
if ($car_count>0){ // Similar by body type
	...
	foreach ($pdo as $row){
		$car_array[$i]['similiar_by']='bodytype';
		foreach($arr_types as $key){ $car_array[$i][$key]=$row[$key];}
		$car_array[$i]['img_folder']=str_replace( "-", "_", mb_strtolower( $row['brand'] ) );
		$car_count--;
		$i++;
...
if ($car_count>0 && $c_price > 100){ // Similar by price (only for cars with real price, not negotiable)
	...
	$pdo = $db->prepare('SELECT * FROM '.$prefx.'_catalog WHERE `price`>=:price_from AND `price`<=:price_to AND `model`<>:model AND `visible`="1" AND `active`="1" ORDER BY RAND() LIMIT :count');
	$pdo->execute(array('price_from' => $price_from, 'price_to' => $price_to, 'model' => $c_model, 'count' => $car_count));
	foreach ($pdo as $row){
		$car_array[$i]['similiar_by']='price';
		foreach($arr_types as $key){ $car_array[$i][$key]=$row[$key];}
		$car_array[$i]['img_folder']=str_replace( "-", "_", mb_strtolower( $row['brand'] ) );
		$car_count--;
		$i++;
...
if ($car_count>0){ // Random
	...
	foreach ($pdo as $row){
		$car_array[$i]['similiar_by']='random';
		foreach($arr_types as $key){ $car_array[$i][$key]=$row[$key];}
		$car_array[$i]['img_folder']=str_replace( "-", "_", mb_strtolower( $row['brand'] ) );
		$car_count--;
		$i++;
