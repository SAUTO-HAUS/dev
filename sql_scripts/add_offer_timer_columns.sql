...
ADD COLUMN `offer_timer` VARCHAR(20) DEFAULT '30:00:00:00' COMMENT 'Timer format: DD:HH:MM:SS' AFTER `advance_amount`,
ADD COLUMN `offer_timer_end` INT DEFAULT 0 COMMENT 'Unix timestamp when offer expires' AFTER `offer_timer`;

-- Update existing on_order cars to have default timer (30 days from now)
UPDATE `gh3sp_car_ctlg` 
SET `offer_timer` = '30:00:00:00', 
