...
-- =====================================================================
-- America (AutoTrader) fixed cost parameters, in EUR.
-- Same shape as the EU/KR params tables. Amounts seeded at 0 on purpose:
-- the real figures are filled in from /parsing/settings.
-- =====================================================================
CREATE TABLE IF NOT EXISTS `gh3sp_parsing_us_params` (
    `id` INT(11) NOT NULL AUTO_INCREMENT,
    `param_key` VARCHAR(100) NOT NULL,
    `value_type` ENUM('fixed','percent') NOT NULL DEFAULT 'fixed' COMMENT 'fixed = amount_eur in EUR, percent = amount_eur is a % of car price',
    `amount_eur` DECIMAL(10,2) NOT NULL DEFAULT 0,
...
-- Same shape as Korea (the client's cost list matches it line for line):
-- sea_freight is part of the CUSTOMS BASE, the rest are flat costs. Customs
-- duty and the luxury tax are computed per car from the price, exactly as on
-- the Korea route — they are not stored here.
INSERT IGNORE INTO `gh3sp_parsing_us_params` (`param_key`, `value_type`, `amount_eur`, `enabled`, `sort_order`) VALUES
('sea_freight',      'fixed', 3000, 1, 1),
('auction_fee',      'fixed', 320,  1, 2),
('delivery_to_port', 'fixed', 325,  1, 3),
('inspection',       'fixed', 150,  1, 4),
('broker_service',   'fixed', 150,  1, 5),
('interpol_check',   'fixed', 100,  1, 6),
('pollution_tax',    'fixed', 75,   1, 7),
('local_tax',       'percent', 2,  1, 8);

-- =====================================================================
-- America (AutoTrader) price markup tiers — mirrors the Korea markup:
-- a flat amount added ON TOP of the EUR-converted car price, by price band.
-- Seeded with the Korea bands and 0 markup; amounts are set in the UI.
-- =====================================================================
CREATE TABLE IF NOT EXISTS `gh3sp_parsing_us_markup_tiers` (
    `id` INT(11) NOT NULL AUTO_INCREMENT,
    `price_from` DECIMAL(12,2) NOT NULL DEFAULT 0,
    `price_to` DECIMAL(12,2) DEFAULT NULL COMMENT 'NULL = no upper limit',
    `markup` DECIMAL(10,2) NOT NULL DEFAULT 0,
...
INSERT INTO `gh3sp_parsing_us_markup_tiers` (`price_from`, `price_to`, `markup`, `sort_order`) VALUES
(0,     10000, 0, 1),
(10001, 15000, 0, 2),
(15001, 20000, 0, 3),
(20001, 30000, 0, 4),
